...
impl<'a> Parser<'a> {
    ...
    pub fn parse_create_view(
        ...
        let auto_refresh =
            if self.parse_keywords(&[Keyword::AUTO, Keyword::REFRESH]) {
    ...
    /// Returns true if the next tokens look like a parenthesized list of
    /// bare column names: `( ident ,` or `( ident )`.
    /// Used to distinguish CTAS column lists from full column definitions.
    fn peek_column_names_only(&self) -> bool {
        if self.peek_token_ref().token != Token::LParen {
            return false;
        }
        matches!(
            (self.peek_nth_token_ref(1).token.clone(), &self.peek_nth_token_ref(2).token),
            (Token::Word(_), Token::Comma | Token::RParen)
        )
    }

    /// Parses a parenthesized list of column names without data types,
    /// e.g. `(col1, col2)` used in Redshift CTAS statements.
    /// Each column is represented as a [ColumnDef] with [DataType::Unspecified].
